...
int main(int argc, char * argv[]) {
    ...
    Config config(72, x_dim, y_dim, z_dim, t_steps, 0.7, 0.06, 0.04, 0.07, 0.03, 0.08, 0.02);
    dvec64 data_3_5_D; 
    dvec64 data_naive;
    ...
    cout << "[**********Compute 3.5D Blocing**********]" << endl;
    t_start = omp_get_wtime();
    stencil_3_5_D.Compute();
    t_end = omp_get_wtime();
    cout << "knl 3.5d blocking compute time " << (t_end - t_start) << endl << endl;
    ...
    t_end = omp_get_wtime();
    cout << "knl naive compute time " << (t_end - t_start) << endl << endl;
 
    cout << "[**********Check**********]" << endl;
    //check with naive knl implementation 
