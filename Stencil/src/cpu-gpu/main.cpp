...
int main(int argc, char * argv[]) {
    ...
    //time blocking steps = 6, please do not modify it
    GPUCPUStencil gpu_cpu_stencil(config, data_gpu_cpu, -1, -1, 6);

    gpu_cpu_stencil.InitData(data_cpu); 
    double t_end = omp_get_wtime();
    cout << "initializing data time: " << (t_end - t_start) << endl << endl;
        
    cout << "[********* GPU-CPU 3.5D Blocking *********]" << endl;
    t_start = omp_get_wtime();
    gpu_cpu_stencil.Compute();
    t_end = omp_get_wtime();
    cout << endl;
    ...
    for(int i = 1; i < z_dim + 1 && check; ++i) {
        for(int j = 1; j < y_dim + 1 && check; ++j) {
           for(int k = 1; k < x_dim + 1; ++k) {
                ...
                if(std::fabs(data_cpu[index] - data_gpu_cpu[index]) >= 1e-6) {
                    cout << i << " " << j << " " << k << " " << data_cpu[index] << " " << data_gpu_cpu[index] << endl; 
                    check = false;
                    break; 
                }
            }
