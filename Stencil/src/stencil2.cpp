...
void Stencil::ComputeXYSubPlane(int x_start, int y_start, int z_size, int y_size, const double *s1, double *s2, vector<vector<vector<vector<double>>>> &cache) {
    ...
    for(int zz = 0; zz < z_dim + 2 * block_t; zz += 2) {
        int z = zz; 
        if(z < z_dim) {
            vector<vector<double>> &plane0 = cache[0][z % 4];
...
#pragma omp simd
        ...
        for(int tt = 1; tt < block_t; ++tt) {
            z = zz - 2 * tt;
            if(z == 0) { 
                const vector<vector<double>> &plane_1 = cache[tt - 1][z % 4];
                const vector<vector<double>> &plane_2 = cache[tt - 1][(z + 1) % 4];
                vector<vector<double>> &plane = cache[tt][z % 4];
...
#pragma omp simd
                ...
                const vector<vector<double>> &plane_0 = cache[tt - 1][(z + 3) % 4];
                const vector<vector<double>> &plane_1 = cache[tt - 1][z % 4];
                const vector<vector<double>> &plane_2 = cache[tt - 1][(z + 1) % 4];
                vector<vector<double>> &plane = cache[tt][z % 4];
                for(int y = tt; y < y_step - tt; ++y) {
...
#pragma omp simd
        ...
        if(z == 0) { 
            const vector<vector<double>> &plane_1 = cache[block_t - 1][z % 4];
            const vector<vector<double>> &plane_2 = cache[block_t - 1][(z + 1) % 4];
            ...
            const vector<vector<double>> &plane_0 = cache[block_t - 1][(z + 3) % 4];
            const vector<vector<double>> &plane_1 = cache[block_t - 1][z % 4];
            const vector<vector<double>> &plane_2 = cache[block_t - 1][(z + 1) % 4];
            for(int y = block_t; y < y_step - block_t; ++y) {
                for(int x = block_t; x < x_step - block_t; ++x) {
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
        for(int i = 0; i < y_blocks * x_blocks; ++i) {
            int j = i / x_blocks;
            int k = i % x_blocks;
            int x_start = block_t + k * (block_x - 2 * block_t);
            int y_start = block_t + j * (block_y - 2 * block_t);
    ...
    //naive handle of remain iterations
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
    for(int i = 1; i < z_dim + 1; ++i) {
        ...
        for(int j = steps; j < y_dim + 2 - steps; ++j) {
            ...
            for(int k = x_dim + 2 - steps; k < x_dim + 1; ++k) {
                int index = i * z_size + j * y_size + k;
                target[index] = source[index];
            }
        }
...
void Stencil::PrintResult() {
    int z_size = (y_dim + 2) * (x_dim + 2);
    int y_size = (x_dim + 2);
    for(int i = 1; i < z_dim + 1; ++i) {
        for(int j = 1; j < y_dim + 1; ++j) {
            for(int k = 1; k < x_dim + 1; ++k) {
                cout << (i * z_size + j * y_size + k) << " " << stencil[i * z_size + j * y_size + k] << endl;
            }
        }
    } 
}
